...
- runtime-neutral Continuation Envelope v2 validation/export/import with schema-v1 Pi capsule compatibility, exact Git and versioned lineage, bounded short explicit summary context with raw transcripts omitted, mnemonic/secret/thinking/tool-output rejection, idle/effect gates, and idempotent fresh-child restore;
...
pi-acp --version  # pi-acp 0.2.10 or newer
```
...
Authenticated Buzz prompts derive task identity only from the canonical relay URL, managed-agent pubkey, channel UUID, and a homogeneous thread root supplied by `buzz-acp`. Each task gets a private mode-0700 Pi session directory. Storage is bounded to 512 task directories and 64 MiB per task; pruning preserves active leases and capsule lineage, and quota exhaustion fails closed. Mixed-thread batches and prompts without authenticated Buzz routing remain ephemeral; adapter restarts reopen only an exact task directory. `pi-continuation-capsule validate|export|reissue|import` uses the bounded runtime-neutral continuation envelope v2 on stdin and never copies raw Pi JSONL between locations. The envelope classifies continuation as `semantic` or `exact`, carries generic runtime lineage, and may include a versioned runtime adapter payload. Schema v1 remains accepted only for compatibility: new exports use v2, existing v1 export receipts remain idempotently replayable, and imports persist v2 lineage metadata. Missing adapter data remains valid for semantic consumers; Pi import fails closed unless the compatible Pi payload is present. Pi adapter v1 supports semantic fresh-child continuation only, so no current adapter may claim exact restore. Import requires the capsule digest supplied by the signed control-plane handoff, verifies clean exact Git, creates a fresh child session, injects one sanitized custom context message, and records only lineage/digest metadata. A delegated import keeps the capsule bound to its source agent while using the authenticated target agent binding for the destination task-session directory; ordinary same-agent imports retain their existing shape.

`pi-delegation prepare-grant|commit-grant|activate-target` implements the host-mounted one-writer activation boundary on macOS and Linux. Delegation protocol v2 offer, accept, READY, and ACTIVATE-grant messages bind the relay/community, thread, owners, distinct source and target managed agents, capsule, exact Git checkpoint, capabilities, and both generations. `prepare-grant` resolves the complete verified decision set through `decisionReadCommand`, samples the source host's current clock after that read, runs one fixed idempotent fence command, and durably records the exact grant before releasing its publish content. A crash can therefore republish the same grant but cannot resume source mutation. `activate-target` invokes its fixed idempotent target command only after an exact source-agent-signed grant whose fence state carries a shared host-HMAC proof loaded through `fenceProofKeyCommand`; offer, accept, READY, or a hand-written agent grant alone never unlocks mutation. A shared host HMAC authenticates the offer-time ownership snapshot and durable fence evidence; payload timestamps are audit data rather than independent wall-clock authority. Source and target state directories fail closed at 512 entries or 64 MiB. Event `signatureVerified` and ownership evidence are accepted only from the cryptographically verifying host boundary. Fixed action commands must remain synchronous foreground operations, must be idempotent, and must not daemonize or leave background effects. Commands inherit no Buzz, model, or Git credential environment.

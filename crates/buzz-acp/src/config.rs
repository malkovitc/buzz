...
pub struct CliArgs {
    ...
    /// Inline team-owned instructions layered after `[Agent Instructions]` and before agent memory.
    #[arg(
        long,
    ...
    /// Read team-owned instructions from a bounded UTF-8 file.
    #[arg(
        long,
...
pub struct Config {
    ...
    pub team_instructions: Option<String>,
    /// Validated file source, re-read for each new ACP session.
    pub team_instructions_file: Option<PathBuf>,
    pub initial_message: Option<String>,
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        } else if let Some(ref path) = team_instructions_file {
...
"#,
    ...
    fn team_instructions_path(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "buzz-acp-{label}-team-instructions-{}.md",
            uuid::Uuid::new_v4()
        ))
    }

    fn team_instructions_args(path: &std::path::Path) -> CliArgs {
        ...
        .expect("file option should parse")
    }

    #[test]
    fn team_instructions_file_loads_and_trims_content() {
        let path = team_instructions_path("valid");
        std::fs::write(&path, "\n  Add members when creating a channel.  \n").unwrap();
        let config = Config::from_args(team_instructions_args(&path)).unwrap();
        ...
        assert!(config.team_instructions_file.is_some());
    }
    ...
    #[test]
    fn invalid_team_instructions_files_fail_startup() {
        let missing = team_instructions_path("missing");
        assert!(Config::from_args(team_instructions_args(&missing)).is_err());

        let oversized = team_instructions_path("oversized");
        std::fs::write(&oversized, vec![b'x'; MAX_TEAM_INSTRUCTIONS_BYTES + 1]).unwrap();
        let error = Config::from_args(team_instructions_args(&oversized))
            .unwrap_err()
            .to_string();
        std::fs::remove_file(oversized).unwrap();
        assert!(error.contains("exceeds 64 KiB limit"), "got: {error}");

        let invalid_utf8 = team_instructions_path("invalid-utf8");
        std::fs::write(&invalid_utf8, [0xff, 0xfe]).unwrap();
        let error = Config::from_args(team_instructions_args(&invalid_utf8))
            .unwrap_err()
            .to_string();

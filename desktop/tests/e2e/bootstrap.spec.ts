...
test.describe("pre-navigation setup", () => {
  ...
  test("reports a failed initial navigation at its source", async ({
    ...
    await expect(
      ...
      /E2E app navigation did not commit to http:\/\/127\.0\.0\.1:1 \(current URL: about:blank\).*Run pnpm test:e2e:smoke or run pnpm build:e2e and check the preview server/,
    );
  });
});

...
fn pilot_runtime_uses_managed_adapter_without_legacy_acp_args() {
    ...
    assert!(runtime.adapter_install_commands[0].contains("/pi-acp-v0.2.8/"));
    assert!(runtime.adapter_install_commands[0].contains("buzz-pi-acp-0.2.8.tgz"));
    assert_eq!(
...
fn rejects_stale_and_accepts_brokered_release() {
    ...
    std::fs::write(&bin, "#!/bin/sh\necho 'pi-acp 0.2.2'\n").expect("write non-durable script");
    assert_eq!(
        pi_adapter_availability(&bin),
        AcpAvailabilityStatus::AdapterOutdated
    );

    std::fs::write(&bin, "#!/bin/sh\necho 'pi-acp 0.2.3'\n").expect("write unreconciled script");
    assert_eq!(
        pi_adapter_availability(&bin),
        AcpAvailabilityStatus::AdapterOutdated
    );
    ...
    std::fs::write(&bin, "#!/bin/sh\necho 'pi-acp 0.2.6'\n").expect("write pre-admission script");
    assert_eq!(
        pi_adapter_availability(&bin),
        AcpAvailabilityStatus::AdapterOutdated
    );
    ...
    std::fs::write(&bin, "#!/bin/sh\necho 'pi-acp 0.2.7'\n")
        .expect("write ephemeral-session script");
    assert_eq!(
        pi_adapter_availability(&bin),
        AcpAvailabilityStatus::AdapterOutdated
    );
